...
namespace Bitrix24\SDK\Services\Sale\Property;
 ...
 * Class Batch
 *
 * Overrides base Batch to handle parameter naming differences in sale.property.* REST methods:
 * - delete uses lowercase 'id' instead of 'ID'
 * - list results are wrapped in 'properties' key and use lowercase 'id'
 *
 * @see https://apidocs.bitrix24.com/api-reference/sale/property/sale-property-delete.html
 * @see https://apidocs.bitrix24.com/api-reference/sale/property/sale-property-list.html
 */
class Batch extends \Bitrix24\SDK\Core\Batch
{
    /**
     * Determines the ID key - lowercase 'id' for order properties
     */
    #[\Override]
    ...
    public function deleteEntityItems(
        ...
        try {
            ...
            foreach ($entityItemId as $cnt => $itemId) {
                if (!is_int($itemId)) {
                    throw new InvalidArgumentException(
                        sprintf(
                            'invalid type "%s" of order property id "%s" at position %s, order property id must be integer type',
                            gettype($itemId),
                            $itemId,
                            $cnt
                        )
            ...
            $errorMessage = sprintf('batch delete order properties: %s', $exception->getMessage());
            $this->logger->error(
                $errorMessage,
                [
