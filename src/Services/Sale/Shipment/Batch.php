...
namespace Bitrix24\SDK\Services\Sale\Shipment;
 ...
 * Class Batch
 *
 * Overrides base Batch to handle parameter naming differences in sale.shipment.* REST methods:
 * - delete uses lowercase 'id' instead of 'ID'
 * - list results are wrapped in 'shipments' key and use lowercase 'id'
 *
 * @see https://apidocs.bitrix24.com/api-reference/sale/shipment/sale-shipment-delete.html
 * @see https://apidocs.bitrix24.com/api-reference/sale/shipment/sale-shipment-list.html
 */
class Batch extends \Bitrix24\SDK\Core\Batch
{
    /**
     * Determines the ID key - lowercase 'id' for shipments
     */
    #[\Override]
    ...
    /**
     * Extracts elements from batch result, unwrapping the 'shipments' key
     */
    #[\Override]
    protected function extractElementsFromBatchResult(ResponseData $responseData, bool $isCrmItemsInBatch): array
    {
        ...
        if (array_key_exists('shipments', $resultData) && is_array($resultData['shipments'])) {
            return $resultData['shipments'];
        }
    ...
    /**
     * Returns reference field path including 'shipments' wrapper for batch query chaining
     */
    #[\Override]
    protected function getReferenceFieldPath(string $prevCommandId, int $lastIndex, string $keyId, bool $isCrmItemsInBatch): string
    {
        return sprintf('$result[%s][shipments][%d][%s]', $prevCommandId, $lastIndex, $keyId);
    }

    /**
     * Get traversable list using lowercase 'id' key and 'shipments' result wrapper
     *
    ...
    public function deleteEntityItems(
        ...
        try {
            ...
            foreach ($entityItemId as $cnt => $itemId) {
                if (!is_int($itemId)) {
                    throw new InvalidArgumentException(
                        sprintf(
                            'invalid type "%s" of shipment id "%s" at position %s, shipment id must be integer type',
                            gettype($itemId),
                            $itemId,
                            $cnt
                        )
            ...
            $errorMessage = sprintf('batch delete shipments: %s', $exception->getMessage());
            $this->logger->error(
                $errorMessage,
                [
