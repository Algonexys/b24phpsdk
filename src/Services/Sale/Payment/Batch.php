 ...
 * Class Batch
 *
 * Overrides base Batch to handle parameter naming differences in sale.payment.* REST methods:
 * - delete uses lowercase 'id' instead of 'ID'
 * - list results are wrapped in 'payments' key and use lowercase 'id'
 *
 * @see https://apidocs.bitrix24.com/api-reference/sale/payment/sale-payment-delete.html
 * @see https://apidocs.bitrix24.com/api-reference/sale/payment/sale-payment-list.html
 */
class Batch extends \Bitrix24\SDK\Core\Batch
{
    /**
     * Determines the ID key - lowercase 'id' for payments
     */
    #[\Override]
    ...
    /**
     * Extracts elements from batch result, unwrapping the 'payments' key
     */
    #[\Override]
    protected function extractElementsFromBatchResult(ResponseData $responseData, bool $isCrmItemsInBatch): array
    {
        ...
        if (array_key_exists('payments', $resultData) && is_array($resultData['payments'])) {
            return $resultData['payments'];
        }
    ...
    /**
     * Returns reference field path including 'payments' wrapper for batch query chaining
     */
    #[\Override]
    protected function getReferenceFieldPath(string $prevCommandId, int $lastIndex, string $keyId, bool $isCrmItemsInBatch): string
    {
        return sprintf('$result[%s][payments][%d][%s]', $prevCommandId, $lastIndex, $keyId);
    }

    /**
     * Get traversable list using lowercase 'id' key and 'payments' result wrapper
     *
    ...
    public function deleteEntityItems(
        ...
        try {
            ...
            foreach ($entityItemId as $cnt => $itemId) {
                if (!is_int($itemId)) {
                    throw new InvalidArgumentException(
                        sprintf(
                            'invalid type "%s" of payment id "%s" at position %s, payment id must be integer type',
                            gettype($itemId),
                            $itemId,
                            $cnt
                        )
            ...
            $errorMessage = sprintf('batch delete payments: %s', $exception->getMessage());
            $this->logger->error(
                $errorMessage,
                [
