...
namespace Bitrix24\SDK\Services\Sale\Cashbox;
 ...
 * Class Batch
 *
 * Overrides base Batch to handle parameter naming differences in sale.cashbox.* REST methods:
 ...
 *
 * @see https://apidocs.bitrix24.com/api-reference/sale/cashbox/sale-cashbox-update.html
 * @see https://apidocs.bitrix24.com/api-reference/sale/cashbox/sale-cashbox-list.html
 */
class Batch extends \Bitrix24\SDK\Core\Batch
{
    /**
     * Get traversable list of cash registers
     *
     * The sale.cashbox.list method does not support pagination and returns all items in one response,
     ...
     * element_id => [
     *  'FIELDS' => [] // cash register fields to update
     * ]
     *
    ...
    public function updateEntityItems(string $apiMethod, array $entityItems): Generator
    {
        ...
        try {
            ...
            foreach ($entityItems as $entityItemId => $entityItem) {
                if (!is_int($entityItemId)) {
                    throw new InvalidArgumentException(
                        sprintf(
                            'invalid type "%s" of cash register id "%s", the id must be integer type',
            ...
            $errorMessage = sprintf('batch update cash registers: %s', $exception->getMessage());
            $this->logger->error(
                $errorMessage,
                [
