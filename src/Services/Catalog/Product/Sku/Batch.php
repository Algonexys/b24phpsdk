...
namespace Bitrix24\SDK\Services\Catalog\Product\Sku;
 ...
 * Class Batch
 *
 * Overrides base Batch to handle differences in catalog.product.sku.* REST methods:
 * - the id key is lowercase 'id', not 'ID' as assumed by the base class default
 * - catalog.product.sku.list wraps list items under the 'units' key instead of returning
...
{
    /**
     * Determines the ID key - lowercase 'id' for catalog.product.sku.* methods
     */
    #[\Override]
    ...
    public function deleteEntityItems(
        ...
        try {
            ...
            foreach ($entityItemId as $cnt => $itemId) {
                if (!is_int($itemId)) {
                    throw new InvalidArgumentException(
                        sprintf(
                            'invalid type «%s» of sku id «%s» at position %s, sku id must be integer type',
                            gettype($itemId),
                            $itemId,
                            $cnt
                        )
            ...
            $errorMessage = sprintf('batch delete sku items: %s', $exception->getMessage());
            $this->logger->error(
                $errorMessage,
                [
