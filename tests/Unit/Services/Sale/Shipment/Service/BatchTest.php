...
namespace Bitrix24\SDK\Tests\Unit\Services\Sale\Shipment\Service;

use Bitrix24\SDK\Core\Contracts\CoreInterface;
use Bitrix24\SDK\Core\Exceptions\InvalidArgumentException;
use Bitrix24\SDK\Core\Result\DeletedItemBatchResult;
use Bitrix24\SDK\Services\Sale\Shipment\Batch as ShipmentBatch;
...
#[CoversClass(ShipmentBatch::class)]
class BatchTest extends TestCase
{
    ...
    public function testAddWrapsFieldsAndReturnsId(): void
    {
        $core = $this->makeCore(static fn (string $apiMethod, array $parameters): array => [
            'shipment' => ['id' => 11] + $parameters['fields'],
        ]);

        $results = iterator_to_array($this->makeBatch($core)->add([['xmlId' => 'a'], ['xmlId' => 'b']]), false);

        $this->assertCount(2, $results);
        $this->assertInstanceOf(AddedShipmentBatchResult::class, $results[0]);
        $this->assertSame(11, $results[0]->getId());
        $this->assertInstanceOf(ShipmentItemResult::class, $results[1]->shipment());
        $this->assertSame(['sale.shipment.add', ['fields' => ['xmlId' => 'a']]], $this->capturedCalls[0]);
        $this->assertSame(['sale.shipment.add', ['fields' => ['xmlId' => 'b']]], $this->capturedCalls[1]);
    }
    ...
    {
        $core = $this->makeCore(static fn (string $apiMethod, array $parameters): array => [
            'shipment' => ['id' => (int)$parameters['id']],
        ]);

        $results = iterator_to_array($this->makeBatch($core)->update([5 => ['xmlId' => 'x']]), false);

        $this->assertCount(1, $results);
        $this->assertInstanceOf(UpdatedShipmentBatchResult::class, $results[0]);
        $this->assertTrue($results[0]->isSuccess());
        $this->assertSame(5, $results[0]->shipment()->id);
        $this->assertSame(['sale.shipment.update', ['id' => '5', 'fields' => ['xmlId' => 'x']]], $this->capturedCalls[0]);
    }
    ...
    public function testDeleteSendsLowercaseId(): void
    {
        ...
        $this->assertTrue($results[0]->isSuccess());
        $this->assertSame(['sale.shipment.delete', ['id' => '7']], $this->capturedCalls[0]);
        $this->assertSame(['sale.shipment.delete', ['id' => '8']], $this->capturedCalls[1]);
    }
    ...
    #[Test]
    #[TestDox('list() unwraps "shipments" result key and adds "id" to select')]
    public function testListUnwrapsResultKeyAndAddsIdToSelect(): void
    {
        ...
        $this->assertInstanceOf(ShipmentItemResult::class, $items[0]);
        $this->assertSame(2, $items[1]->id);
        $this->assertSame('sale.shipment.list', $this->capturedCalls[0][0]);
        $this->assertSame(['xmlId', 'id'], $this->capturedCalls[0][1]['select']);
        $this->assertSame(['id' => 'ASC'], $this->capturedCalls[0][1]['order']);
    }

    #[Test]
    #[TestDox('list() chains batch queries by the last "id" in the "shipments" result key')]
    public function testListChainsBatchQueriesByLastId(): void
    {
        ...
        $this->assertSame(['>id' => '$result[cmd_0][shipments][49][id]'], $this->capturedCalls[2][1]['filter']);
    }

    private function makeBatch(CoreInterface $core): Batch
    {
        return new Batch(new ShipmentBatch($core, new NullLogger()), new NullLogger());
    }
}
