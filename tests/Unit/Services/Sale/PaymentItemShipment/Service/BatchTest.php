...
namespace Bitrix24\SDK\Tests\Unit\Services\Sale\PaymentItemShipment\Service;

use Bitrix24\SDK\Core\Contracts\CoreInterface;
use Bitrix24\SDK\Core\Exceptions\InvalidArgumentException;
use Bitrix24\SDK\Core\Result\DeletedItemBatchResult;
use Bitrix24\SDK\Services\Sale\PaymentItemShipment\Batch as PaymentItemShipmentBatch;
...
#[CoversClass(PaymentItemShipmentBatch::class)]
class BatchTest extends TestCase
{
    ...
    public function testAddWrapsFieldsAndReturnsId(): void
    {
        $core = $this->makeCore(static fn (string $apiMethod, array $parameters): array => [
            'paymentItemShipment' => ['id' => 11] + $parameters['fields'],
        ]);

        $results = iterator_to_array($this->makeBatch($core)->add([['xmlId' => 'a'], ['xmlId' => 'b']]), false);

        $this->assertCount(2, $results);
        $this->assertInstanceOf(PaymentItemShipmentAddedBatchResult::class, $results[0]);
        $this->assertSame(11, $results[0]->getId());
        $this->assertInstanceOf(PaymentItemShipmentItemResult::class, $results[1]->paymentItemShipment());
        $this->assertSame(['sale.paymentitemshipment.add', ['fields' => ['xmlId' => 'a']]], $this->capturedCalls[0]);
        $this->assertSame(['sale.paymentitemshipment.add', ['fields' => ['xmlId' => 'b']]], $this->capturedCalls[1]);
    }
    ...
    {
        $core = $this->makeCore(static fn (string $apiMethod, array $parameters): array => [
            'paymentItemShipment' => ['id' => (int)$parameters['id']],
        ]);

        $results = iterator_to_array($this->makeBatch($core)->update([5 => ['xmlId' => 'x']]), false);

        $this->assertCount(1, $results);
        $this->assertInstanceOf(PaymentItemShipmentUpdatedBatchResult::class, $results[0]);
        $this->assertTrue($results[0]->isSuccess());
        $this->assertSame(5, $results[0]->paymentItemShipment()->id);
        $this->assertSame(['sale.paymentitemshipment.update', ['id' => '5', 'fields' => ['xmlId' => 'x']]], $this->capturedCalls[0]);
    }
    ...
    public function testDeleteSendsLowercaseId(): void
    {
        ...
        $this->assertTrue($results[0]->isSuccess());
        $this->assertSame(['sale.paymentitemshipment.delete', ['id' => '7']], $this->capturedCalls[0]);
        $this->assertSame(['sale.paymentitemshipment.delete', ['id' => '8']], $this->capturedCalls[1]);
    }
    ...
    #[Test]
    #[TestDox('list() unwraps "paymentItemsShipment" result key and adds "id" to select')]
    public function testListUnwrapsResultKeyAndAddsIdToSelect(): void
    {
        $core = $this->makeCore(static fn (string $apiMethod, array $parameters): array => [
            'paymentItemsShipment' => [['id' => 1], ['id' => 2], ['id' => 3]],
        ]);

        $items = iterator_to_array($this->makeBatch($core)->list(['xmlId'], ['>id' => 0], [], 2), false);

        $this->assertCount(2, $items);
        $this->assertInstanceOf(PaymentItemShipmentItemResult::class, $items[0]);
        $this->assertSame(2, $items[1]->id);
        $this->assertSame('sale.paymentitemshipment.list', $this->capturedCalls[0][0]);
        $this->assertSame(['xmlId', 'id'], $this->capturedCalls[0][1]['select']);
        $this->assertSame(['id' => 'ASC'], $this->capturedCalls[0][1]['order']);
    }

    #[Test]
    #[TestDox('list() chains batch queries by the last "id" in the "paymentItemsShipment" result key')]
    public function testListChainsBatchQueriesByLastId(): void
    {
        $core = $this->makeCore(
            static function (string $apiMethod, array $parameters): array {
                ...
                return [
                    'paymentItemsShipment' => array_map(
        ...
        $this->assertSame(['>id' => '$result[cmd_0][paymentItemsShipment][49][id]'], $this->capturedCalls[2][1]['filter']);
    }

    private function makeBatch(CoreInterface $core): Batch
    {
        return new Batch(new PaymentItemShipmentBatch($core, new NullLogger()), new NullLogger());
    }
}
