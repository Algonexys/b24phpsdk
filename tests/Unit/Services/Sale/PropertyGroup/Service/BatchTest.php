...
namespace Bitrix24\SDK\Tests\Unit\Services\Sale\PropertyGroup\Service;

use Bitrix24\SDK\Core\Contracts\CoreInterface;
use Bitrix24\SDK\Core\Exceptions\InvalidArgumentException;
use Bitrix24\SDK\Core\Result\DeletedItemBatchResult;
use Bitrix24\SDK\Services\Sale\PropertyGroup\Batch as PropertyGroupBatch;
...
#[CoversClass(PropertyGroupBatch::class)]
class BatchTest extends TestCase
{
    ...
    public function testAddWrapsFieldsAndReturnsId(): void
    {
        $core = $this->makeCore(static fn (string $apiMethod, array $parameters): array => [
            'propertyGroup' => ['id' => 11] + $parameters['fields'],
        ]);

        $results = iterator_to_array($this->makeBatch($core)->add([['xmlId' => 'a'], ['xmlId' => 'b']]), false);

        $this->assertCount(2, $results);
        $this->assertInstanceOf(PropertyGroupAddBatchResult::class, $results[0]);
        $this->assertSame(11, $results[0]->getId());
        $this->assertInstanceOf(PropertyGroupItemResult::class, $results[1]->propertyGroup());
        $this->assertSame(['sale.propertygroup.add', ['fields' => ['xmlId' => 'a']]], $this->capturedCalls[0]);
        $this->assertSame(['sale.propertygroup.add', ['fields' => ['xmlId' => 'b']]], $this->capturedCalls[1]);
    }
    ...
    {
        $core = $this->makeCore(static fn (string $apiMethod, array $parameters): array => [
            'propertyGroup' => ['id' => (int)$parameters['id']],
        ]);

        $results = iterator_to_array($this->makeBatch($core)->update([5 => ['xmlId' => 'x']]), false);

        $this->assertCount(1, $results);
        $this->assertInstanceOf(PropertyGroupUpdateBatchResult::class, $results[0]);
        $this->assertTrue($results[0]->isSuccess());
        $this->assertSame(5, $results[0]->propertyGroup()->id);
        $this->assertSame(['sale.propertygroup.update', ['id' => '5', 'fields' => ['xmlId' => 'x']]], $this->capturedCalls[0]);
    }
    ...
    public function testDeleteSendsLowercaseId(): void
    {
        ...
        $this->assertTrue($results[0]->isSuccess());
        $this->assertSame(['sale.propertygroup.delete', ['id' => '7']], $this->capturedCalls[0]);
        $this->assertSame(['sale.propertygroup.delete', ['id' => '8']], $this->capturedCalls[1]);
    }
    ...
    #[Test]
    #[TestDox('list() unwraps "propertyGroups" result key and adds "id" to select')]
    public function testListUnwrapsResultKeyAndAddsIdToSelect(): void
    {
        $core = $this->makeCore(static fn (string $apiMethod, array $parameters): array => [
            'propertyGroups' => [['id' => 1], ['id' => 2], ['id' => 3]],
        ]);

        $items = iterator_to_array($this->makeBatch($core)->list(['xmlId'], ['>id' => 0], [], 2), false);

        $this->assertCount(2, $items);
        $this->assertInstanceOf(PropertyGroupItemResult::class, $items[0]);
        $this->assertSame(2, $items[1]->id);
        $this->assertSame('sale.propertygroup.list', $this->capturedCalls[0][0]);
        $this->assertSame(['xmlId', 'id'], $this->capturedCalls[0][1]['select']);
        $this->assertSame(['id' => 'ASC'], $this->capturedCalls[0][1]['order']);
    }

    #[Test]
    #[TestDox('list() chains batch queries by the last "id" in the "propertyGroups" result key')]
    public function testListChainsBatchQueriesByLastId(): void
    {
        ...
        $this->assertSame(['>id' => '$result[cmd_0][propertyGroups][49][id]'], $this->capturedCalls[2][1]['filter']);
    }

    private function makeBatch(CoreInterface $core): Batch
    {
        return new Batch(new PropertyGroupBatch($core, new NullLogger()), new NullLogger());
    }
}
